...
void Steper() {
  ...
  minRPM = map(adcCurrent, 0, 1024, 100, maxRPM);
  // minRPM = 900;
  ...
  if (now - lastReadServo >= checkTime) {
    ...
    if (RPMok == 1) {
      ...
      if (CheckTime == 0) {
        ...
        if (currentRPM <= (maxRPM - rpmThresholdmax)) {
          Up = 3;
          stepMove = 1;
          CheckTime = 1;
          angle += 2;
      ...
      // if (currentRPM <= minRPM && lowRpmCheck == 0) {
      //   angleStart = 0;
      //   lowRpmCheck = 1;
      //   Down = 0;
      //   Up = 0;
